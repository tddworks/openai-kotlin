 ...
 * content block of type "text". The following input messages are equivalent:
 *
 * {"role": "user", "content": "Hello, Claude"} {"role": "user", "content":
...
class MessageTest {
    ...
    fun `should serialize an array of content blocks`() {
        ...
        // When
        val result = Json.encodeToString(Message.serializer(), userMessage)
    ...
    fun `should serialize single line message content`() {
        ...
        val assistantMessage =
            Message(role = Role.Assistant, content = Content.TextContent(text = "message"))

        // When
        val result = Json.encodeToString(Message.serializer(), assistantMessage)
    ...
    fun `should return assistant message name`() {
        ...
        assertEquals("message", (assistantMessage.content as Content.TextContent).text)
    }
    ...
    fun `should return user message name`() {
        ...
        assertEquals("message", (userMessage.content as Content.TextContent).text)
    }
}

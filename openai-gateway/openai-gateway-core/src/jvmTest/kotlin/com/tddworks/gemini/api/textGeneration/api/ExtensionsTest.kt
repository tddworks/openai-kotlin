...
class ExtensionsTest {
            ...
            GenerateContentResponse(
                candidates =
                    listOf(
                        Candidate(
                            content =
                                Content(parts = listOf(Part.TextPart("some-text")), role = "model")
                        )
                    ),
                usageMetadata =
                    UsageMetadata(
    ...
    fun `should convert ChatCompletionRequest to GeminiGenerateContentRequest`() {
        ...
        // When
        val generateContentRequest = chatCompletionRequest.toGeminiGenerateContentRequest()
        ...
        assertEquals("Hello", (generateContentRequest.contents[0].parts[0] as Part.TextPart).text)

...
class GeminiITest : AutoCloseKoinTest() {
    ...
    fun `should return generateContent response with image`() = runTest {
        ...
        val image = javaClass.classLoader.getResource("origami.png")?.readBytes()
            ?.toBase64()
        val response = gemini.generateContent(
            GenerateContentRequest(
                contents = listOf(
                    Content(
                        parts = listOf(
                            Part.TextPart(text = "Detect items, with no more than 20 items. Output a json list where each entry contains the 2D bounding box in \"box_2d\" and a text label in \"label\"."),
                            Part.InlineDataPart(
